...
extension UIView {
    ...
    var sd: SDLayoutSwift {
        get {
            if let rs = objc_getAssociatedObject(self, &layoutKey) as? SDLayoutSwift {
                return rs
            } else {
        ...
        set {
            objc_setAssociatedObject(self, &layoutKey, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
